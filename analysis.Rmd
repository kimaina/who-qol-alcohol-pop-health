...
packages =c( "dplyr",  "readxl","Hmisc","Gmisc", "magrittr", "flextable", "MASS", "tidyverse", "caret", "knitr", "kableExtra","xtable", "stargazer", "ggpubr", "haven", "tidycomm", "gtable", "ggpmisc")
...
```{r fig.align='center', align = 'center', warning=FALSE}
...
drinking_patterns.long=drinking_patterns %>% 
  pivot_longer(
           cols = starts_with("Drinks per day"),names_to = "month",values_to = "drinks_per_days",values_drop_na = TRUE) %>%
            mutate(
...
subset <- sample_n(drinking_patterns, 20)$ID
drinking_patterns.few = drinking_patterns.long[drinking_patterns.long$ID %in% subset,]
ggplot(drinking_patterns.few, aes(x = month, y = drinks_per_days)) + 
   geom_point()+
    geom_line(aes(group = ID))+
...
drinking_patterns.long%>% ggplot(aes(x = month, y = drinks_per_days)) +
  geom_jitter(aes(group = ID),color=drinking_patterns.long$ID)+
    guides(colour=FALSE) +
    labs(y = "Average Drinks per day Per Month", x="Month")+
     guides(fill=FALSE)+
...
drinking_patterns.long.no = drinking_patterns.long%>% filter(drinks_per_days < 10)
drinking_patterns.long.no%>%ggplot(aes(x = month, y = drinks_per_days)) +
...
The dotted line is the cohort level regression and as expected it has a negative slope - indicating an overall reduction in average number of dayly drinks per month. The reduction is not as substancial as the above (Drinking Days Per Month)
...
## Bivariate Analysis | Density Plots
